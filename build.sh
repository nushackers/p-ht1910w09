...
make_offline_mirror() {
    ...
    local incpkg=$(cat ${script_path}/incpkg.x86_64)
    if  [[ -n ${incpkg} ]]; then
        echo " => Downloading incpkg packages to offline mirror."
        sh -c "pacman --noconfirm --dbpath /tmp/ -Syu -w --cachedir ${offline_mirror_path} $(echo ${incpkg} | sed ':a;N;$!ba;s/\n/ /g')"
    fi
...
while getopts 'N:V:L:P:A:D:w:o:g:vh' arg; do
    case "${arg}" in
        ...
        o) out_dir="${OPTARG}" ;;
        g) gpg_key="${OPTARG}" ;;
        v) offline_mirror_path=${script_path}/${work_dir}/x86_64/airootfs/srv/http/arch_offline
verbose="-v" ;;
        h) _usage 0 ;;
        *)
